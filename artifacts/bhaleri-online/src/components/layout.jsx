...
export default function Layout({ children }) {
  ...
  const { data: notifications } = useListNotifications({
    enabled: !!user && isEnabled("notifications"),
    refetchInterval: 60000,
    refetchIntervalInBackground: false,
  });
  const unreadCount = notifications?.filter((n) => !n.isRead).length ?? 0;

  const { data: cartItems } = useGetCart({ enabled: !!user && isEnabled("marketplace") });
  ...
  const navLinks = [
    ...
    { label: "Village Map", href: "/map", featureKey: "map" },
    { label: "About", href: "/about", featureKey: "about" },
  ].filter((l) => !l.featureKey || isEnabled(l.featureKey));
