import { useListNotifications, useMarkAllNotificationsRead, useMarkNotificationRead } from "@/lib/api";
...
export default function Notifications() {
  ...
  const { data: notifications, isLoading } = useListNotifications({ enabled: !!user, refetchInterval: 30000 });
  ...
  const handleMarkAll = () => {
    markAll.mutate(undefined, { onSuccess: () => qc.invalidateQueries({ queryKey: ["listNotifications"] }) });
  };

  const handleClickNotif = (id, isRead, link) => {
    if (!isRead) markOne.mutate({ id }, { onSuccess: () => qc.invalidateQueries({ queryKey: ["listNotifications"] }) });
        ...
        <div className="space-y-3">
          {[1,2,3].map((i) => (
            <div key={i} className="flex gap-3 p-4 border rounded-xl">
              ...
              <div className="flex-1 space-y-2"><Skeleton className="h-4 w-full" /><Skeleton className="h-3 w-1/3" /></div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {notifications.map((n) => (
            <button
              ...
              className={`w-full flex gap-3 p-4 rounded-xl border text-left transition-colors hover:bg-muted/50 ${!n.isRead ? "bg-primary/5 border-primary/20" : "bg-background"}`}
            >
              <div className="relative shrink-0">
                <Avatar className="w-10 h-10">
                  <AvatarImage src={n.fromUserAvatar || ""} />
                  <AvatarFallback className="text-sm">{(n.fromUserName || "?").charAt(0).toUpperCase()}</AvatarFallback>
                </Avatar>
