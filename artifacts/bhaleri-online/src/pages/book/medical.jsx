...
import { useCreateBooking, useListMedical, useListProviders } from "@/lib/api";
import { useLocation } from "wouter";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Card, CardContent } from "@/components/ui/card";
import { Stethoscope, ChevronRight, IndianRupee, Clock } from "lucide-react";
...
export default function BookMedical() {
  ...
  const { data: medicals = [] } = useListMedical();
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <Card>
          <CardContent className="pt-5 space-y-3">
            ...
            {medicals.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">No clinics available.</p>
            ) : (
              <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
                {medicals.map(clinic => (
                  ...
                  >
                    ...
                    {clinic.type && <div className="text-xs text-muted-foreground mt-0.5">{clinic.type}</div>}
              ...
              <div className="grid grid-cols-3 gap-2">
                {TIME_SLOTS.map(slot => (
                  <button
                    ...
                    className={`flex items-center justify-center gap-1 px-2 py-2 rounded-xl border-2 text-xs font-medium transition-colors ${timeSlot === slot ? "border-primary bg-primary/5" : "border-border hover:bg-muted/30"}`}
                  >
        ...
        <Card className="bg-primary/5 border-primary/20">
          <CardContent className="pt-4 pb-4">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xl font-bold text-primary">₹{CONSULT_FEE}</span>
            </div>
          </CardContent>
        </Card>
