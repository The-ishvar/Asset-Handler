...
import { useGetUserProfile, useToggleFollow, useListReels, useListListings } from "@workspace/api-client-react";
...
import { Package, Play, MessageCircle, UserCheck, UserPlus, Phone, Mail } from "lucide-react";
...
export default function PublicProfile() {
  ...
  const { data: reels } = useListReels({ userId: profileId }, { query: { enabled: !!profileId } });
  const { data: listings } = useListListings({ userId: profileId }, { query: { enabled: !!profileId } });
  ...
  const handleFollow = () => {
    ...
    toggleFollow.mutate({ id: profileId }, {
      ...
      onError: () => { setIsFollowing(isFollowing); setFollowerCount(followerCount); }
    });
  };
