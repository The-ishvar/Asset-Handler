...
import {
  useListListings, useListReels, usePatchMyProfile,
  useUpdateUser, ListingStatus
...
import {
  Package, Settings, LogOut, Shield, Plus, Play,
  Edit3, Camera, Phone, Mail
} from "lucide-react";
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    updateUser.mutate(
      { id: user.id, data: { name, phone } },
      {
        onSuccess: (updated) => {
          patchProfile.mutate(
            { data: { bio: bio || null, avatarUrl: avatarUrl || null, coverUrl: coverUrl || null } },
            {
              onSuccess: () => {
                if (token) login(token, { ...updated, avatarUrl: avatarUrl || null } as any);
                setIsEditing(false);
    ...
    <div className="max-w-3xl mx-auto space-y-0">
      ...
      <div className="relative h-40 md:h-56 bg-gradient-to-br from-primary/30 to-emerald-400/30 rounded-b-none rounded-xl overflow-hidden">
        {(user as any).coverUrl && (
          <img src={(user as any).coverUrl} alt="Cover" className="w-full h-full object-cover" />
        )}
      ...
      <div className="relative px-4 pb-4 border-b">
        ...
        <div className="mt-3 space-y-1">
          ...
          {(user as any).bio && (
            <p className="text-muted-foreground text-sm">{(user as any).bio}</p>
          )}
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  placeholder="Tell the Bhaleri community about yourself..."
                  rows={2}
                />
              </div>
              <div className="space-y-1.5">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="listings" className="p-4 mt-0">
          <div className="flex justify-between items-center mb-4">
            ...
            <Button size="sm" asChild>
              <Link href="/buy-sell/new"><Plus className="w-3.5 h-3.5 mr-1" /> New</Link>
            </Button>
          </div>
          {listingsLoading ? (
        ...
        <TabsContent value="reels" className="p-4 mt-0">
          <div className="flex justify-between items-center mb-4">
            ...
            <Button size="sm" asChild>
              <Link href="/reels/new"><Plus className="w-3.5 h-3.5 mr-1" /> New</Link>
            </Button>
          </div>
          {reelsLoading ? (
              ...
              {myReels.map(reel => (
                <Link key={reel.id} href="/reels">
                  <div className="aspect-[9/16] bg-black rounded-lg overflow-hidden relative cursor-pointer group">
                    ...
                    <div className="absolute bottom-1 left-1 right-1 text-white text-xs font-medium line-clamp-1 drop-shadow">
                      {reel.title}
                    </div>
                  </div>
                </Link>
          ...
          <div className="max-w-md space-y-4">
            <div className="p-4 bg-muted/30 rounded-xl space-y-3">
              ...
              <div>
                ...
                <div className="font-medium">{new Date((user as any).createdAt).toLocaleDateString("en-IN", { year: "numeric", month: "long" })}</div>
              </div>
            </div>
