...
import {
  useListReels, useToggleReelLike, useRecordReelView,
  useListReelComments, useAddReelComment, Reel
...
import {
  Heart, MessageCircle, Share2, Eye, Plus, Volume2, VolumeX,
  Send, X, Play, Pause
} from "lucide-react";
...
function ReelCard({ reel, isActive }: { reel: Reel; isActive: boolean }) {
  ...
  const { data: comments } = useListReelComments(reel.id, { query: { enabled: showComments } });
  ...
  const handleComment = (e: React.FormEvent) => {
    ...
    addComment.mutate(
      { id: reel.id, data: { content: commentText } },
      {
        onSuccess: () => {
          setCommentText("");
    ...
    <div className="relative w-full h-full bg-black flex items-center justify-center select-none">
      ...
      <div className="absolute right-3 bottom-24 flex flex-col items-center gap-5">
        <button onClick={handleLike} className="flex flex-col items-center gap-1 text-white">
          <div className={`p-2 rounded-full transition-colors ${liked ? "bg-red-500" : "bg-black/30"}`}>
        ...
        <button
          onClick={() => { navigator.share?.({ title: reel.title, url: window.location.href }).catch(() => {}); }}
...
export default function Reels() {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[80vh] text-center space-y-4">
        ...
        <p className="text-muted-foreground max-w-xs">
          Be the first to share a short video with the Bhaleri community!
        </p>
        {user && (
          <Link href="/reels/new">
            <Button><Plus className="w-4 h-4 mr-2" /> Create Reel</Button>
          </Link>
        )}
      </div>
    );
  ...
  return (
    <div className="relative -mx-4 -my-6 md:-my-8">
      ...
      {user && (
        <Link href="/reels/new">
          <button className="absolute top-4 right-4 z-30 bg-primary text-primary-foreground rounded-full p-3 shadow-lg">
      ...
      {/* Reel label */}
      <div className="absolute top-4 left-4 z-30 text-white font-bold text-lg drop-shadow">
        🎬 Reels
      </div>
      ...
      >
        {reels.map((reel, i) => (
          <div
            key={reel.id}
            style={{ scrollSnapAlign: "start", height: "calc(100vh - 56px - 64px)" }}
          >
      ...
      <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex flex-col gap-1 z-20">
        {reels.map((_, i) => (
          <div
            key={i}
            className={`rounded-full transition-all ${i === activeIndex ? "w-2 h-4 bg-white" : "w-1.5 h-1.5 bg-white/40"}`}
          />
        ))}
      </div>
    </div>
