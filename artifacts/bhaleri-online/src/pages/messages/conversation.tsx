...
import { useGetConversation, useSendMessage, useGetUser } from "@workspace/api-client-react";
...
export default function Conversation() {
  ...
  const { data: messages, isLoading } = useGetConversation(otherId, {
    query: { enabled: !!user && !!otherId, refetchInterval: 5000 }
  });
  const { data: otherUser } = useGetUser(otherId, { query: { enabled: !!otherId } });
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    if (!text.trim()) return;
    sendMsg.mutate(
      { data: { receiverId: otherId, content: text } },
      {
        onSuccess: () => {
          setText("");
    ...
    <div className="max-w-2xl mx-auto flex flex-col" style={{ height: "calc(100vh - 56px - 64px)" }}>
      ...
      <div className="flex items-center gap-3 py-3 border-b shrink-0">
        ...
        <Avatar className="w-9 h-9">
          <AvatarImage src={(otherUser as any)?.avatarUrl || ""} />
